...
const editorMachine = xstate.setup({
  ...
  actions: {
    ...
    "assign behaviors": xstate.assign({
      behaviors: ({
        event
      }) => (xstate.assertEvent(event, "update behaviors"), /* @__PURE__ */ new Set([...foundationalBehaviors, ...event.behaviors]))
    }),
  ...
  context: ({
    ...
    behaviors: /* @__PURE__ */ new Set([...foundationalBehaviors, ...input.behaviors ?? behavior_core.coreBehaviors]),
...
function EventListenerPlugin(props) {
  const $ = reactCompilerRuntime.c(4), editor = useEditor(), on = useEffectEvent.useEffectEvent(props.on);
  let t0, t1;
