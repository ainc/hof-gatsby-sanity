...
const oneLineBehaviors = [
  ...
  behavior_core.defineBehavior({
    ...
    guard: ({
      ...
      const focusTextBlock = selector_isAtTheStartOfBlock.getFocusTextBlock({
        context
      }), selectionStartPoint = selector_getSelectionStartPoint.getSelectionStartPoint({
        context
      ...
      if (!focusTextBlock || !utils_index.isTextBlock(context, event.block) || !selectionStartPoint || !selectionEndPoint)
        return !1;
      const blockStartPoint = util_isEqualSelectionPoints.getBlockStartPoint(focusTextBlock), blockEndPoint = util_isEqualSelectionPoints.getBlockEndPoint(focusTextBlock), newFocus = util_isEqualSelectionPoints.getBlockEndPoint({
        node: event.block,
        path: [{
          _key: event.block._key
        }]
      });
      return util_isEqualSelectionPoints.isEqualSelectionPoints(blockStartPoint, selectionStartPoint) && util_isEqualSelectionPoints.isEqualSelectionPoints(blockEndPoint, selectionEndPoint) ? {
        focusTextBlock,
        newFocus
      } : !1;
    },
  ...
  behavior_core.defineBehavior({
    ...
    guard: ({
      ...
      }), newFocus = util_isEqualSelectionPoints.getBlockEndPoint({
        node: targetBlock,
        path: [{
