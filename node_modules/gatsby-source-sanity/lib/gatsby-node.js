...
const mapCrossDatasetReferences_1 = require("./util/mapCrossDatasetReferences");
...
const initializePlugin = async ({ reporter }, pluginOptions) => {
    ...
    try {
        ...
        if (config._mocks) {
            ...
            api = await (0, remoteGraphQLSchema_1.getRemoteGraphQLSchema)(client, config);
        }
        ...
        const typeMap = (0, remoteGraphQLSchema_1.getTypeMapFromGraphQLSchema)(api, config.typePrefix);
...
const sourceNodes = async (args, pluginConfig) => {
    ...
    const lastBuildTime = await args.cache.get((0, cache_1.getCacheKey)(config, cache_1.CACHE_KEYS.LAST_BUILD));
    ...
    await args.cache.set((0, cache_1.getCacheKey)(config, cache_1.CACHE_KEYS.LAST_BUILD), new Date().toISOString());
};
